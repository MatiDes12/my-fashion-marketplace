...
// Utility function to sanitize text for SEO (prevent XSS)
const sanitizeForSEO = (text: string): string => {
  if (!text) return '';
  return text
...
// Component to handle dynamic SEO meta tags
const DynamicHead = ({ store, owner }: { store: StoreSettings | null; owner: any }) => {
  if (!store || !owner) return null;

  // Sanitize and generate fallback SEO data if not configured
  const storeName = sanitizeForSEO(store.name || 'Store');
  const storeDescription = sanitizeForSEO(store.shortDescription || store.description || '');
  
  const metaTitle = sanitizeForSEO(
    store.seo?.metaTitle || `${storeName} - Quality Products | Ethiopian Marketplace`
  ).substring(0, 60);
  
  const metaDescription = sanitizeForSEO(
    store.seo?.metaDescription || 
    `Shop at ${storeName}. ${storeDescription || 'Quality products with fast delivery across Ethiopia.'} Trusted verified seller.`
  ).substring(0, 160);
  
  const keywords = sanitizeForSEO(
    store.seo?.keywords || `${storeName.toLowerCase()}, ethiopian store, online shopping ethiopia, quality products, fast delivery`
  );
  
  // Use slug if available, otherwise fallback to owner.id
  const storeUrl = store.slug || owner.id?.replace(/[^a-zA-Z0-9-_]/g, '') || '';
  
  // Generate structured data for better SEO with sanitized values
  const structuredData = {
    "@context": "https://schema.org",
    "@type": "Store",
    "name": storeName,
    "description": storeDescription,
    "url": `https://avrioxshop.com/store/${storeUrl}`,
    "logo": store.logo_url ? sanitizeForSEO(store.logo_url) : undefined,
    "image": store.banner_url ? sanitizeForSEO(store.banner_url) : undefined,
    "address": {
      "@type": "PostalAddress",
      "addressLocality": sanitizeForSEO(store.address?.city || ''),
      "addressRegion": sanitizeForSEO(store.address?.subCity || ''),
      "streetAddress": sanitizeForSEO(
        `${store.address?.houseNo || ''} ${store.address?.kebele ? `Kebele ${store.address.kebele}` : ''} ${store.address?.wereda ? `Wereda ${store.address.wereda}` : ''}`.trim()
      ),
      "addressCountry": "ET"
    },
    "telephone": sanitizeForSEO(store.phone || ''),
    "openingHours": Object.entries(store.workingHours || {})
      .filter(([_, hours]) => hours.isOpen)
      .map(([day, hours]) => sanitizeForSEO(`${day.charAt(0).toUpperCase() + day.slice(1)} ${hours.open}-${hours.close}`))
      .filter(Boolean),
    "paymentAccepted": Object.entries(store.payment_methods || {})
      .filter(([_, accepted]) => accepted)
      .map(([method]) => method === 'cash' ? 'Cash' : method.toUpperCase())
      .filter(method => /^[A-Z]+$/.test(method)), // Only allow uppercase letters
    "priceRange": "$$"
  };

  // Validate and sanitize image URLs
  const sanitizedBannerUrl = store.banner_url && /^https?:\/\//.test(store.banner_url) ? store.banner_url : undefined;
  const sanitizedLogoUrl = store.logo_url && /^https?:\/\//.test(store.logo_url) ? store.logo_url : undefined;
  ...
  return (
    <Head>
      ...
      <meta property="og:url" content={`https://avrioxshop.com/store/${storeUrl}`} />
      <meta property="og:site_name" content="Avrio" />
      {sanitizedBannerUrl && <meta property="og:image" content={sanitizedBannerUrl} />}
      {sanitizedLogoUrl && <meta property="og:logo" content={sanitizedLogoUrl} />}
      
      {/* Twitter Card */}
      <meta name="twitter:card" content="summary_large_image" />
      <meta name="twitter:title" content={metaTitle} />
      <meta name="twitter:description" content={metaDescription} />
      {sanitizedBannerUrl && <meta name="twitter:image" content={sanitizedBannerUrl} />}
      
      {/* Structured Data - Only include if we have valid data */}
      {storeName && (
        <script
      ...
      {/* Canonical URL */}
      <link rel="canonical" href={`https://avrioxshop.com/store/${storeUrl}`} />
    </Head>
  );
};
...
export default function StorePage() {
  ...
  return (
    <>
      ...
      <DynamicHead store={store} owner={owner} />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="relative -mt-24 sm:-mt-32 pb-8">
          <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8">
            ...
            {/* SEO Preview Section */}
            {store.seo && (store.seo.metaTitle || store.seo.metaDescription) && (
              <div className="mt-8">
                <h2 className="text-lg font-semibold text-gray-900 mb-4">🔍 How this store appears in Google</h2>
                <div className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                  <div className="text-blue-600 text-lg hover:underline cursor-pointer">
                    {store.seo.metaTitle || `${store.name} - Quality Products | Ethiopian Marketplace`}
                  </div>
                  <div className="text-green-700 text-sm">
                    avrioxshop.com/store/{store.slug || owner.id}
                  </div>
                  <div className="text-gray-600 text-sm mt-1">
                    {store.seo.metaDescription || `Shop at ${store.name}. ${store.shortDescription || store.description || 'Quality products with fast delivery across Ethiopia.'} Trusted verified seller.`.substring(0, 160)}
                  </div>
                </div>
              </div>
            )}
